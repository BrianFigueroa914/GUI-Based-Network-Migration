...
const LOCAL_KEY = "nm_profiles_hci_v2";
...
export default function Profiles(){
  ...
  function addNew(){ setSel(-1); setDraft(empty()); }
  function save(){
    if(!ok) return;
    if(sel >= 0){
      const next = [...profiles]; next[sel] = draft; setProfiles(next);
    }else{
      setProfiles([...profiles, draft]); setSel(profiles.length);
    }
  }
  function remove(){
    if(sel < 0) return;
  ...
  function exportJSON(){ download(JSON.stringify(profiles, null, 2), "profiles.json"); }
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          ...
          <ul className="list" role="listbox" aria-label="Saved profiles">
            {filtered.map((p, i) => {
              const idx = profiles.indexOf(p); // keep index relative to full list
              const isSel = idx === sel;
              return (
                <li
                  key={idx}
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      ...
                      <div className="muted" style={{marginTop:2}}>
                        <span className={`badge ${p.mode==="DHCP"?"success":"warn"}`} style={{marginRight:6}}>
                          {p.mode}
                        </span>
